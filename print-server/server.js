...
const WS_PORT = parseInt(process.env.PRINT_SERVER_PORT || "9090");
...
function startServer() {
  ...
  wss.on("error", (err) => {
    if (err.code === "EADDRINUSE") {
      console.error(`[Print Server] Port ${WS_PORT} already in use. Killing existing process and retrying in 2s...`);
      // Kill whatever is on that port then retry
      exec(`powershell -Command "Stop-Process -Id (Get-NetTCPConnection -LocalPort ${WS_PORT} -State Listen | Select-Object -ExpandProperty OwningProcess) -Force -ErrorAction SilentlyContinue"`,
  ...
  wss.on("listening", () => {
    console.log(`[Print Server] Listening on ws://localhost:${WS_PORT}`);
    // Auto-detect printers AFTER server is up, then log config
