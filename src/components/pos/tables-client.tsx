...
export function TablesClient({ tables, waiterId, hasOpenShift, openedByName, canBypassShift, currencySymbol }: TablesClientProps) {
  ...
  const [confirmMerge, setConfirmMerge] = useState<{ open: boolean; table: TableWithOrders } | null>(null);
  ...
  const handleMergeAll = (table: TableWithOrders) => {
    if (table.orders.length < 2) return;
    setConfirmMerge({ open: true, table });
  };

  const doMergeAll = async (table: TableWithOrders) => {
  ...
  return (
    <div className="h-full overflow-auto p-4 md:p-6">
      {/* Shift status badge — right-aligned, fit-content */}
      <div className="flex justify-end mb-3">
        {shiftLocked ? (
          <Link
            href="/pos/shift"
            className="inline-flex items-center gap-1.5 bg-red-50 border border-red-200 rounded-full px-3 py-1.5 text-xs font-semibold text-red-700 hover:bg-red-100 transition-colors shadow-sm"
          >
            <Lock className="w-3.5 h-3.5 text-red-500" />
            No open shift — tap to open
          </Link>
        ) : openedByName ? (
          <span className="inline-flex items-center gap-1.5 bg-green-50 border border-green-200 rounded-full px-3 py-1.5 text-xs font-semibold text-green-700 shadow-sm">
            <span className="w-2 h-2 rounded-full bg-green-500 shrink-0" />
            Shift open · {openedByName}
          </span>
        ) : null}
      </div>
      ...
      {confirmMerge && (
        <ConfirmDialog
          ...
          title={`Merge ${confirmMerge.table.orders.length} orders into one?`}
          description="All split orders on this table will be combined into a single bill."
          confirmLabel="Merge"
          variant="default"
          onConfirm={() => doMergeAll(confirmMerge.table)}
